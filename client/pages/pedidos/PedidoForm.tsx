...
import { AlertTriangle, ChevronsUpDown, Check, X, Save, Link, AlertCircle } from "lucide-react";

const schema = z.object({
  estabelecimento_id: z.number({ required_error: "Estabelecimento é obrigatório" }),
  tipo_pedido: z.enum(["Atendente", "QR Code", "APP", "Outro"], { required_error: "Tipo de Pedido é obrigatório" }),
...
export default function PedidoForm({
  ...
  const [selectedExtras, setSelectedExtras] = useState<{ item_id: number; categoria_id: number; quantidade: number; valor_unitario_centavos: number }[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      if (estResp?.data) {
        const ordered = estResp.data.sort((a: Estabelecimento, b: Estabelecimento) => (a.data_cadastro < b.data_cadastro ? 1 : -1));
      ...
      if (cliResp?.data) {
        const ordered = cliResp.data.sort((a: Cliente, b: Cliente) => a.nome.localeCompare(b.nome));
        setClientes(ordered);
        if (!pedido) setValue("cliente_id", null);
      }
      // Cardápios: asc by nome
      const cardResp = await makeRequest("/api/cardapios?page=1&limit=1000");
      if (cardResp?.data) setCardapios(cardResp.data.sort((a: Cardapio, b: Cardapio) => a.nome.localeCompare(b.nome)));
      // Categorias e Itens (para extras)
      const catResp = await makeRequest("/api/itens-categorias?page=1&limit=1000");
      if (catResp?.data) setCategorias(catResp.data.sort((a: ItemCategoria, b: ItemCategoria) => a.nome.localeCompare(b.nome)));
      const itensResp = await makeRequest("/api/itens?page=1&limit=1000");
      if (itensResp?.data) setItens(itensResp.data.sort((a: Item, b: Item) => a.nome.localeCompare(b.nome)));
    } catch (e) {
  ...
  const valorExtras = useMemo(() => {
    return selectedExtras.reduce((sum, e) => sum + e.quantidade * e.valor_unitario_centavos, 0);
  }, [selectedExtras]);
  ...
  const onSubmit = (data: FormData) => {
    if (selectedCardapios.length === 0) {
      toast({ title: "Erro de validação", description: "Selecione pelo menos um Cardápio", variant: "destructive" });
      return;
    }
  ...
  const hasPrerequisites = estabelecimentos.length > 0 && clientes.length > 0 && cardapios.length > 0;
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="text-sm text-yellow-800">
                {estabelecimentos.length === 0 ? (
                  <>
                    ...
                    <button onClick={() => window.open("/estabelecimentos", "_blank")} className="text-blue-600 hover:text-blue-800 underline inline-flex items-center gap-1">
                      <Link className="w-3 h-3" /> (ir para módulo Estabelecimentos)
                    </button>
                  </>
                ) : clientes.length === 0 ? (
                  <>
                    Antes de cadastrar, é necessário ter pelo menos um Cliente. {" "}
                    <button onClick={() => window.open("/clientes", "_blank")} className="text-blue-600 hover:text-blue-800 underline inline-flex items-center gap-1">
                  ...
                  <>
                    ...
                    <button onClick={() => window.open("/cardapios", "_blank")} className="text-blue-600 hover:text-blue-800 underline inline-flex items-center gap-1">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                  ...
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" className="w-full justify-between foodmax-input">
                      {watchedValues.estabelecimento_id
                        ? estabelecimentos.find((e) => e.id === watchedValues.estabelecimento_id)?.nome
                        : "Selecione"}
                      ...
                      <CommandList>
                        <CommandGroup>
                          {estabelecimentos.map((e) => (
                            <CommandItem key={e.id} onSelect={() => setValue("estabelecimento_id", e.id)}>
                              <Check className={cn("mr-2 h-4 w-4", watchedValues.estabelecimento_id === e.id ? "opacity-100" : "opacity-0")} />
                              {e.nome}
                            </CommandItem>
                          ))}
                        </CommandGroup>
                ...
                {errors.estabelecimento_id && (
                  <span className="text-sm text-red-600">{errors.estabelecimento_id.message as any}</span>
                )}
              </div>

              <div>
                <Label>Tipo de Pedido *</Label>
                <Select value={watchedValues.tipo_pedido} onValueChange={(v) => setValue("tipo_pedido", v as any)}>
                ...
                {errors.tipo_pedido && <span className="text-sm text-red-600">{errors.tipo_pedido.message as any}</span>}
              </div>
              ...
              <div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" className="w-full justify-between foodmax-input">
                      {watchedValues.cliente_id ? clientes.find((c) => c.id === watchedValues.cliente_id)?.nome : "Não Cliente"}
                      <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-full p-0">
                      ...
                      <CommandList>
                        <CommandGroup>
                          <CommandItem onSelect={() => setValue("cliente_id", null)}>
                            <Check className={cn("mr-2 h-4 w-4", watchedValues.cliente_id == null ? "opacity-100" : "opacity-0")} />
                            Não Cliente
                          </CommandItem>
                          {clientes.map((c) => (
                            <CommandItem key={c.id} onSelect={() => setValue("cliente_id", c.id)}>
                              <Check className={cn("mr-2 h-4 w-4", watchedValues.cliente_id === c.id ? "opacity-100" : "opacity-0")} />
                              {c.nome}
                            </CommandItem>
                          ))}
                        </CommandGroup>
              ...
              <div className="md:col-span-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" className="w-full justify-between foodmax-input">
                      {selectedCardapios.length > 0 ? `${selectedCardapios.length} selecionado(s)` : "Selecione Cardápios"}
                      <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-full p-0">
                      ...
                      <CommandList>
                        <CommandGroup>
                          {cardapios.map((c) => (
                            ...
                            >
                              <Check className={cn("mr-2 h-4 w-4", selectedCardapios.includes(c.id) ? "opacity-100" : "opacity-0")} />
                              <span className="flex-1">{c.nome}</span>
                              <Badge variant="secondary">{formatCurrencyBRL(c.preco_total_centavos)}</Badge>
                            </CommandItem>
                          ))}
                        </CommandGroup>
                      </CommandList>
              ...
              <div className="md:col-span-2">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select value={selectedCategoriaId ? String(selectedCategoriaId) : undefined} onValueChange={(v) => setSelectedCategoriaId(parseInt(v))}>
                      <SelectTrigger className="foodmax-input">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button
                          ...
                          className={cn("w-full justify-between foodmax-input", !selectedCategoriaId && "opacity-60 cursor-not-allowed")}
                        >
                          {selectedCategoriaId ? "Selecionar Itens Extra" : "Selecione uma categoria"}
                          <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-full p-0">
                                ...
                                return (
                                  ...
                                  >
                                    <div className="flex items-center justify-between w-full">
                                      <div className="flex items-center gap-2">
                                        {isLow && <AlertCircle className="w-4 h-4 text-yellow-600" />}
                                        <span>{item.nome}</span>
                                      </div>
                                      <span className="text-xs text-gray-500">Estoque: {item.estoque_atual ?? 0}</span>
                                    </div>
                                  </CommandItem>
                                );
                              })}
                ...
                {selectedCategoriaId && filteredExtras.some((i) => (i.estoque_atual || 0) < 3) && (
                ...
                {selectedExtras.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {selectedExtras.map((ex) => {
                      ...
                      return (
                        <div key={ex.item_id} className="flex items-center justify-between bg-gray-50 rounded p-2">
                          <div>
                            <div className="font-medium flex items-center gap-2">
                              {((item.estoque_atual || 0) < 3) && <AlertCircle className="w-4 h-4 text-yellow-600" />} {item.nome}
                            </div>
                            <div className="text-xs text-gray-600">Estoque Atual: {item.estoque_atual ?? 0}</div>
                          </div>
                          <div className="flex items-center gap-2">
                            <div className="flex flex-col gap-1">
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  setSelectedExtras((prev) =>
                                    prev.map((p) => (p.item_id === ex.item_id ? { ...p, quantidade: Math.max(1, parseInt(e.target.value) || 1) } : p)),
                                  )
                                }
                                disabled={isZero}
                                  ...
                                  setSelectedExtras((prev) =>
                                    prev.map((p) =>
                                      p.item_id === ex.item_id
                                        ? { ...p, valor_unitario_centavos: Math.round(parseFloat(e.target.value || "0") * 100) }
                                        : p,
                                    ),
                                  )
                                }
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              <div className="text-sm font-medium w-20 text-center bg-white p-1 rounded border">{formatCurrencyBRL(total)}</div>
                            </div>
                            <Button
                              type="button"
                              variant="outline"
                              size="sm"
                              onClick={() => setSelectedExtras((prev) => prev.filter((p) => p.item_id !== ex.item_id))}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setValorTotalMask(e.target.value === "" ? "" : formatInputCurrency(cents));
                ...
                {errors.valor_total_centavos && (
                  <span className="text-sm text-red-600">{errors.valor_total_centavos.message}</span>
                )}
              </div>
              <div>
                ...
                <Input
                  type="datetime-local"
                  value={watchedValues.data_hora_finalizado ? new Date(watchedValues.data_hora_finalizado).toISOString().slice(0, 16) : ""}
                  onChange={(e) => setValue("data_hora_finalizado", e.target.value ? new Date(e.target.value).toISOString() : null)}
                  className="foodmax-input"
                />
              </div>
              <div className="md:col-span-2">
                <Label>Observação</Label>
                <Textarea rows={3} {...register("observacao")} className="foodmax-input resize-none" placeholder="Observações..." />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg border">
            <div className="w-60">
              <Label>Status</Label>
              <Select value={watchedValues.status} onValueChange={(v) => setValue("status", v as any)}>
                <SelectTrigger className="foodmax-input">
        ...
        <DialogFooter>
          ...
          <Button type="submit" onClick={handleSubmit(onSubmit)} disabled={isLoading || !hasPrerequisites} className="bg-foodmax-orange hover:bg-orange-600">
