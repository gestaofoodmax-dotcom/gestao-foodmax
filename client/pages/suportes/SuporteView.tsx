...
export function SuporteView({ isOpen, onClose, suporte, onReplied }: SuporteViewProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="w-[85vw] h-[90vh] max-w-none overflow-y-auto" onInteractOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="text-xl sm:text-2xl font-normal py-2">Visualizar Suporte</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-foodmax-orange">Suporte</h2>
                <div className="text-sm text-gray-900">{suporte.titulo}</div>
              </div>
            </div>
            <div className="text-right">
              <Badge className={`${STATUS_BADGE_STYLES[suporte.status] || "bg-gray-50 text-gray-700 border border-gray-200"}`}>{suporte.status}</Badge>
              <p className="text-xs text-gray-800 mt-1">Cadastrado em {formatDate(suporte.data_cadastro)}</p>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg border">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div>
                  <Badge className={`${PRIORIDADE_BADGE_STYLES[suporte.prioridade]}`}>{suporte.prioridade}</Badge>
                </div>
              </div>
              <div>
          ...
          <div className="bg-white p-4 rounded-lg border">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="text-sm font-medium text-gray-600">Data de Cadastro</div>
                <div className="text-sm text-gray-900">{formatDate(suporte.data_cadastro)}</div>
              </div>
              <div>
                <div className="text-sm font-medium text-gray-600">Data de Atualização</div>
                <div className="text-sm text-gray-900">{formatDate(suporte.data_atualizacao)}</div>
              </div>
              <div>
              ...
              {suporte.resposta_admin && (
                <div className="col-span-2">
                  <div className="text-sm font-medium text-gray-600">Resposta Admin</div>
                  <div className="text-sm text-gray-900 whitespace-pre-wrap">{suporte.resposta_admin}</div>
                </div>
              )}
            </div>
          </div>
