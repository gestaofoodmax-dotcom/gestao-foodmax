...
export default function Dashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const clientesResp = await makeRequest(
          `/api/clientes?page=1&limit=100`,
        );
        const clientes = Array.isArray(clientesResp?.data)
          ? clientesResp.data
          : [];
  ...
  useEffect(() => {
    ...
    const filtroEst = (arr: any[]) =>
      selectedEst === "todos"
        ? arr
        : arr.filter((x) => String(x.estabelecimento_id) === selectedEst);
    const now = new Date();
    ...
    const pedidosFinalizadosMes = pedidosFiltered.filter(
      (p) => p.status === "Finalizado" && isSameMonth(p.data_cadastro),
    );
    const pedidosTotal =
      pedidosFinalizadosMes.reduce(
        (sum, p) => sum + (p.valor_total_centavos || 0),
        0,
      ) / 100;
        ...
        <main className="flex-1 p-6">
          <div className="max-w-7xl mx-auto">
            ...
            {(() => {
              ...
              const cadastro = user?.data_cadastro
              ...
              if (!hasPay && cadastro) {
                const diffDays = Math.floor(
                  (today.getTime() - cadastro.getTime()) /
                    (1000 * 60 * 60 * 24),
                );
                if (diffDays === 4) {
                ...
                if (base) {
                  ...
                  if (diffDays === 29) {
                    ...
                    desc =
                      "Renove agora e continue utilizando todos os recursos.";
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-800">
                      R$ {cards.pedidosTotal.toFixed(2)}
                    </p>
                    <p className="text-xs text-gray-500">
                      {cards.pedidosCount} pedidos
                    </p>
                  </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-orange-500">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {cards.clientesAtivos} clientes ativos
                    </p>
                  </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                {(() => {
                  const filtro =
                    selectedEst === "todos"
                      ? recentPedidos
                      : recentPedidos.filter(
                          (p) => String(p.estabelecimento_id) === selectedEst,
                        );
                  const ordered = [...filtro]
                    .sort((a, b) =>
                      ...
                      {ordered.map((p) => (
                        ...
                        >
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500">
                                {new Date(p.data_cadastro).toLocaleString(
                                  "pt-BR",
                                )}
                              </p>
                            </div>
                          ...
                          <div className="text-sm font-semibold text-gray-800">
                            R$ {(p.valor_total_centavos / 100).toFixed(2)}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                {(() => {
                  const filtro =
                    selectedEst === "todos"
                      ? recentClientes
                      : recentClientes.filter(
                          (c) => String(c.estabelecimento_id) === selectedEst,
                        );
                  const ordered = [...filtro]
                    .sort((a, b) =>
                      a.data_cadastro < b.data_cadastro ? 1 : -1,
